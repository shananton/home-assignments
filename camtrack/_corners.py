...
def draw(grayscale_image: np.ndarray, corners: FrameCorners, colorize: bool = False) -> np.ndarray:
    """
    Draw corners on image.

    :param grayscale_image: grayscale float32 image.
    :param corners: corners to draw, pyramid levels must be less than 7.
    :param colorize: color the corners depending on their ids
    ...
    for id, point, block_size in zip(corners.ids, corners.points, corners.sizes):
        ...
        cv2.circle(bgr, point, radius, tuple(np.random.default_rng(seed=id).uniform(size=3)) if colorize else (0, 1, 0))
    return bgr
...
def create_cli(build):
    ...
    @click.option('--colorize', is_flag=True)
    def cli(frame_sequence, file_to_load, file_to_dump, show, colorize):
        """
        ...
        if show:
            ...
            while True:
                ...
                bgr = draw(grayscale, corner_storage[frame], colorize)
